...
export const putIngredients = async (req, res, next) => {
    try {
        let ingredient = await Fridge.findById( req.body.editId );
        ...
        let ingredientToEdit = await Fridge.findOneAndUpdate(
            {_id: ingredient._id},
            ingredientUpdate,
            {new: true}
        );
        res.status(200).json({ data: "Fridge updated", ingredientToEdit });
    } catch (error) {
        console.log(error);
    }
    next();
