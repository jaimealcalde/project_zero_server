...
export const putIngredients = async (req, res, next) => {
    try {
        ...
        let ingredientUpdate = {
            _id: ingredient._id,
            product: ingredient.product || req.body.editId.product,
            description: ingredient.description || req.body.editId.description,
            gluten: ingredient.gluten || req.body.editId.gluten,
            lactose: ingredient.lactose || req.body.editId.lactose
        }
        console.log("new2ingredientUpdate)
    } catch (error) {
        console.log(error);
    }
    next();
