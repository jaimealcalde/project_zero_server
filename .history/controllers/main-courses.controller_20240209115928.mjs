...
export const putIngredients = async (req, res, next) => {
    try {
        let ingredient = await Main.findById( req.body.editId );
        ...
        let ingredientToEdit = await Main.findOneAndUpdate(
            {_id: ingredient._id},
            ingredientUpdate,
            {new: true}
        );
        res.status(200).json({ data: "Main updated", ingredientToEdit });
    } catch (error) {
        console.log(error);
    }
    next();
