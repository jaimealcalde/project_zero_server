import Entrees from "../models/entrees.model.mjs";

//?GET ALL INGREDIENTS ingredients.get("/", getIngredients);

export const getIngredients = async (req, res, next) => {
    try {
        const ingredients = await Entrees.find();
...
export const putIngredients = async (req, res, next) => {
    try {
        let ingredient = await Entrees.findById( req.body.editId );
        ...
        let ingredientToEdit = await Entrees.findOneAndUpdate(
            {_id: ingredient._id},
            ingredientUpdate,
            {new: true}
        );
        res.status(200).json({ data: "Entrees updated", ingredientToEdit });
    } catch (error) {
        console.log(error);
    }
    next();
...
export const deleteIngredients = async (req, res, next) => {
    try {
        ...
        let ingredient = await Entrees.findById( req.params.entreeId);
        await ingredient.deleteOne();
        res.json({ message: "Entree deleted" });
    } catch (error) {
        console.log(error);
    }
    next();
